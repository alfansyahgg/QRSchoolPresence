...
public class ScanPresensiHadirActivity extends AppCompatActivity {

    ActivityScanPresensiHadirBinding binding;
    SPHelper spHelper;
    Integer userId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityScanPresensiHadirBinding.inflate(getLayoutInflater());
        ...
        codeScanner.setDecodeCallback(new DecodeCallback() {
            ...
            public void onDecoded(@NonNull Result result) {
                ...
                if(!jenis.equals("hadir")){
                    runOnUiThread(() -> {
                        Toast.makeText(ScanPresensiHadirActivity.this, "Gagal : Bukan Presensi Hadir", Toast.LENGTH_SHORT).show();
                        finish();
                    });
                }else{
                    if(currentTimestamp > startTimestamp && currentTimestamp < expiredTimestamp){
                        submitPresensi("hadir", md5);
                    }else if( currentTimestamp > expiredTimestamp && currentTimestamp <= expiredTimestamp + (120*60*1000)){
                        submitPresensi("terlambat", md5);
                    }else if( currentTimestamp > expiredTimestamp + (120*60*1000)){
                        runOnUiThread(() -> {
                            Toast.makeText(ScanPresensiHadirActivity.this, "Gagal : Presensi Sudah Tidak Berlaku", Toast.LENGTH_SHORT).show();
                            finish();
                        });
                    }
                }
        ...
        codeScanner.setErrorCallback(new ErrorCallback() {
            ...
            public void onError(@NonNull Throwable thrown) {
                runOnUiThread(() -> {
                    Toast.makeText(ScanPresensiHadirActivity.this, "Camera Initialization Error : " + thrown.getMessage(), Toast.LENGTH_LONG).show();
                });
            }
        });
        ...
        service.presensi(jenis, userId, kode).enqueue(new Callback<PresensiModel>() {
            ...
            public void onResponse(Call<PresensiModel> call, Response<PresensiModel> response) {
                if(response.isSuccessful()){
                    ...
                    Toast.makeText(ScanPresensiHadirActivity.this, "" + presensiModel.getMessage(), Toast.LENGTH_SHORT).show();
                    finish();
                }
            }
